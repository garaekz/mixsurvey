...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create mixsurvey-6d48
  fly apps create mixsurvey-6d48-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app mixsurvey-6d48
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app mixsurvey-6d48-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'mixsurvey-6d48-staging' does not match app name in config file 'mixsurvey-6d48'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name mixsurvey-6d48-db
  fly postgres attach --app mixsurvey-6d48 mixsurvey-6d48-db

  fly postgres create --name mixsurvey-6d48-staging-db
  fly postgres attach --app mixsurvey-6d48-staging mixsurvey-6d48-staging-db
  ```
